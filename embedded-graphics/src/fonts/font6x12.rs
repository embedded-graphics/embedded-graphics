...
where
    C: PixelColor,
{
    fn render_str(text: &'a str) -> Font6x12<'a, C> {
        Self {
            pos: Coord::new(0, 0),
            text,
            style: Style::default(),
...
impl<'a, C> WithStyle<C> for Font6x12<'a, C>
where
    C: PixelColor,
{
...
impl<'a, C> Drawable for Font6x12<'a, C> where C: PixelColor {}
    ...
    C: PixelColor,
{
    ...
    /// a new `Font6x12`. For a mutating transform, see `translate_mut`.
    ///
    /// ```
...
mod tests {
    ...
    fn text_dimensions() {
        ...
        assert_eq!(hello.dimensions(), UnsignedCoord(72, 12));
