//! 8x12 pixel font. Image data taken from the [Uzebox Wiki page](http://uzebox.org/wiki/Font_Bitmaps)

use super::super::drawable::*;
use super::super::transform::*;
use super::Font;

const FONT_IMAGE: &[u8] = include_bytes!("../../data/font8x12_1bpp.raw");
...
impl<'a> Font<'a> for Font8x12<'a> {
    fn render_str(text: &'a str) -> Font8x12<'a> {
        Self { pos: (0, 0), text }
    }
}

#[derive(Debug, Clone, Copy)]
pub struct Font8x12Iterator<'a> {
    char_walk_x: u32,
    char_walk_y: u32,
...
impl<'a> IntoIterator for &'a Font8x12<'a> {
    type IntoIter = Font8x12Iterator<'a>;
    type Item = Pixel;
...
impl<'a> Iterator for Font8x12Iterator<'a> {
    type Item = Pixel;
...
impl<'a> Drawable for Font8x12<'a> {}

impl<'a> Transform for Font8x12<'a> {
    /// Translate the image from its current position to a new position by (x, y) pixels, returning
    /// a new `Font8x12`.
    ///
    /// ```
    /// # use embedded_graphics::fonts::{ Font, Font8x12 };
    /// # use embedded_graphics::transform::Transform;
    ///
    /// // 8px x 1px test image
    /// let text = Font8x12::render_str("Hello world");
