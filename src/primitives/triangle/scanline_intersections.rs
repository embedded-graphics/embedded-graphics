...
impl ScanlineIntersections {
    ...
    pub fn new(
        ...
        if let Some(lines) =
            Self::generate_lines(triangle, stroke_width, stroke_offset, has_fill, scanline_y)
        {
            Self {
                lines,
                has_fill,
    ...
    fn generate_lines(
            ...
            core::iter::from_fn(move || {
                ...
                while idx < 3 {
                    ...
                    if let Some(next_segment) =
                        ThickSegment::new(start, end).intersection(scanline_y)
                    {
        ...
        if triangle.is_collapsed(stroke_width, stroke_offset)
    ...
    pub fn reset_with_new_scanline(&mut self, scanline_y: i32) {
        if let Some(lines) = Self::generate_lines(
            &self.triangle,
            self.stroke_width,
            self.stroke_offset,
            self.has_fill,
